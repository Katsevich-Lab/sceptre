...
construct_negative_control_pairs <- function(n_calibration_pairs, calibration_group_size, grna_assignments, response_matrix, n_nonzero_trt_thresh, n_nonzero_cntrl_thresh, grna_group_data_frame, response_grna_group_pairs) {
  ...
  out <- compute_nt_nonzero_matrix_and_n_ok_pairs(j = response_matrix@j,
                                                  p = response_matrix@p,
  ...
  n_nonzero_tot <- as.integer(colSums(n_nonzero_m))
