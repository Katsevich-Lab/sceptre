    ...
    setup_data = function(data, params) {
        if ("group" %in% names(data)) {
            ...
            max_unique_pts_per_group <- data |>
                ...
                dplyr::summarise(
                    unique_pts_per_group = length(unique(pts_per_group))
                ) |>
        ...
        withr::with_seed(4, {
            data |>
                ...
                dplyr::mutate(
                    ...
                    ymin = quantile_fun(stats::qbeta(
                        p = (1 - ci_level) / 2,
                        shape1 = r,
                        shape2 = dplyr::n() + 1 - r
                    )),
                    ymax = quantile_fun(stats::qbeta(
                        p = (1 + ci_level) / 2,
                        shape1 = r,
                        shape2 = dplyr::n() + 1 - r
                    ))
...
stat_qq_band <- function(
    ...
    ggplot2::layer(
        ...
        params = list(
            ...
            max_pts_to_plot = max_pts_to_plot,
            ci_level = ci_level,
            ...
        )
    )
        ...
        withr::with_seed(4, {
            data |>
                ...
                dplyr::mutate(
                    ...
                    # apply the formula
                    x = quantile_fun(stats::ppoints(dplyr::n())[rank(
                        y,
                        ties.method = "first"
                    )]),
                    # threshold
