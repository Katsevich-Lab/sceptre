construct_data_frame_v2 <- function(curr_df, curr_response_result, output_amount) {
  ...
  se_over_root_n <- vapply(curr_response_result, FUN = function(l) l$se_over_root_n, FUN.VALUE = numeric(1))
  curr_df$lower_ci_log_2_fold_change <- log(fold_change - 1.96 * se_over_root_n)/log(2)
  curr_df$upper_ci_log_2_fold_change <- log(fold_change + 1.96 * se_over_root_n)/log(2)
  if (output_amount >= 2L) {
