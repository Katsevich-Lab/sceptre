...
get_idx_vector_factory <- function(
    calibration_check,
    indiv_nt_grna_idxs,
    grna_group_idxs,
    low_moi
    ...
    f <- function(curr_grna_group) {
        if (calibration_check) {
            get_idx_vector_calibration_check(
                curr_grna_group,
                indiv_nt_grna_idxs,
                low_moi
            )
...
get_idx_vector_calibration_check <- function(
    curr_grna_group,
    indiv_nt_grna_idxs,
    low_moi
) {
    undercover_nts <- strsplit(x = curr_grna_group, split = "&", fixed = TRUE)[[
        1
    ]]
...
get_idx_vector_discovery_analysis <- function(
    curr_grna_group,
    grna_group_idxs
) {
...
perm_test_glm_factored_out <- function(
    synthetic_idxs,
    B1,
    B2,
    B3,
...
discovery_ntcells_perm_test <- function(
    synthetic_idxs,
    B1,
    B2,
    B3,
    ...
    for (i in seq_along(grna_groups)) {
        curr_grna_group <- grna_groups[i]
        idxs <- get_idx_vector_discovery_analysis(
...
crt_glm_factored_out <- function(
    ...
    for (i in seq_along(response_ids)) {
        ...
        pieces_precomp <- compute_precomputation_pieces(
            ...
            covariate_matrix = covariate_matrix,
            fitted_coefs = response_precomputations[[
                curr_response_id
            ]]$fitted_coefs,
...
discovery_ntcells_crt <- function(
    B1,
    B2,
    B3,
    fit_parametric_curve,
