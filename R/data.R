...
#' contain the coordinate and transcription start site position of each gene
#' relative to reference genome GRCh38 and GRCh37, respectively. Both
#' `gene_position_data_frame_grch38` and `gene_position_data_frame_grch37` were
#' constructed from reference genomes available on the 10x Genomics website. The
#' GRCh38 reference genome has been used by 10x Cell Ranger since 2020.
#'
...
#'
#' The gRNA target data frame corresponding to the example high MOI data. The
#' data frame contains the columns `grna_id` (ID of an individual gRNA),
#' `grna_target` (genomic target of the gRNA), `chr` (target chromosome),
#' `start` (start coordinate of target location), and `end` (end coordinate of
#' target location).
