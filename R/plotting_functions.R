...
#' # See the example in the run_sceptre_lowmoi help file.
#' ?run_sceptre_lowmoi
compare_calibration_and_discovery_results <- function(calibration_result, discovery_result,
                                                      alpha = 0.1, multiple_testing_correction = "BH",
  ...
  p_out <- ggplot2::ggplot(data = df, mapping = ggplot2::aes(y = p_value, col = lab)) +
    stat_qq_points(ymin = 1e-8, size = 0.55, alpha = 0.8) +
    stat_qq_band() +
    ggplot2::labs(x = "Expected null p-value", y = if (include_y_axis_text) "Observed p-value" else "") +
    ggplot2::geom_abline(col = "black") +
    get_my_theme() +
...
#' # See the example in the run_sceptre_lowmoi help file.
#' ?run_sceptre_lowmoi
make_volcano_plot <- function(discovery_result, alpha = 0.1, x_limits = c(-1.5, 1.5), transparency = 0.5, point_size = 0.55, multiple_testing_correction = "BH") {
  discovery_result <- discovery_result |> stats::na.omit()
  ...
  out <- ggplot2::ggplot(data = discovery_result |> dplyr::mutate(reject = p_value < p_thresh,
    ...
    ggplot2::ggtitle("Discovery pairs volcano plot")
  return(out)
}


plot_discovery_result <- function(sceptre_object) {
  # compute n rejections
  n_rejections <- obtain_discovery_set(sceptre_object@discovery_result, alpha, multiple_testing_correction)


  p1 <- compare_calibration_and_discovery_results(calibration_result = sceptre_object@calibration_result,
                                                  discovery_result = sceptre_object@discovery_result,
                                                  transform_scale = FALSE, include_legend = TRUE,
                                                  include_y_axis_text = TRUE)
  p2 <- compare_calibration_and_discovery_results(calibration_result = sceptre_object@calibration_result,
                                                  discovery_result = sceptre_object@discovery_result,
                                                  transform_scale = TRUE, include_legend = FALSE,
                                                  include_y_axis_text = FALSE)
  p3 <- make_volcano_plot(discovery_result = sceptre_object@discovery_result)

  disc_set <- obtain_discovery_set(sceptre_object@discovery_result, alpha, multiple_testing_correction)
  p_out <- cowplot::plot_grid(p1, p2, p3, NULL, labels = c("a", "b", "c"), rel_heights = c(0.55, 0.45), nrow = 2)
}
