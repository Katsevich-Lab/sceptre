...
`sceptre` is an R package that facilitates statistically rigorous, massively scalable, and user-friendly single-cell CRISPR screen data analysis. First, users should install `sceptre`. Users also should install the `sceptredata` package, which contains the example data used in this vignette.
...
Users can load `sceptre` and `sceptredata` via a call to `library()`.
...
sceptre_object <- sceptre_object |> # |> is R's base pipe, similar to %>%
  set_analysis_parameters(discovery_pairs, positive_control_pairs) |>
  run_calibration_check(print_progress = FALSE, parallel = TRUE) |>
  run_power_check(print_progress = FALSE, parallel = TRUE) |>
  run_discovery_analysis(print_progress = FALSE, parallel = TRUE)
```
