...
Tim Barry & Eugene Katsevich, **Updated March 2022**
...
**Note**: Users optionally can implement their own strategy for producing the perturbation matrix.

## Step 3: Combine gRNAs

The third step is to combine the perturbations of gRNAs that target the same chromosomal location. Information about the genomic position that each gRNA targets is available in the `gRNA_groups_table`, which we explore below.
...
The negative control pairs were constructed by pairing each non-targeting gRNA group to the entire set of genes, as is standard. When a large number of genes and non-targeting gRNA groups is available, it is reasonable to analyze a random subset of the non-targeting gRNA group-gene pairs to reduce computational cost. We recommend analyzing at least as many negative control pairs as candidate pairs.

**Note**: the `pair_type` column, while helpful for bookkeeping purposes, is optional.

**Note**: Negative control and positive control pairs are *not* required to run `sceptre`; however, negative and positive control pairs are extremely useful for checking method calibration and power (see Step 7, below) and therefore are strongly recommended.
...
*Trans* relationships are more complex (and their direction less certain) than *cis* relationships, as *trans* relationships typically involve one or more transcription factors. We recommend two-sided tests for all tests of *trans* relationships.
...
**Note**: Users can supply the raw gRNA expression matrix they obtained from Step 1 or the perturbation matrix they obtained from Step 2 to the `combined_perturbation_matrix` argument, in which case `sceptre` treats each gRNA as its own group of one. In this case the `gRNA_group` column still should be a subset of the row names of `combined_perturbation_matrix`. We recommend against this usage unless the data consist of only a single gRNA per target.

## Step 7. Analyze the results

The output is a data frame called `result`; `result` is simply `gene_gRNA_pairs` with two additional columns: `p_value`, and `z_value`. `p_value` is the *p*-value of the association test for a given pair, and `z_value` is the z-value of a negative binomial GLM fitted to the data for a given pair. Positive z-values indicate increased expression and negative z-values indicate decreased expression. We examine several rows of `result` below.
...
It is a good idea to verify that the test is correctly calibrated. To this end we create a QQ-plot of the *p*-values corresponding to the negative control pairs. We extract the negative control *p*-values from the `result` data frame and pass them to the function `make_qq_plot`.
...
The positive control *p*-values are all small ($< 10^{-10}$), indicating that the test is powerful.
