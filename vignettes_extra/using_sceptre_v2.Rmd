...
The function `combine_gRNAs` takes as arguments a thresholded `gRNA_matrix` (i.e., the `perturbation_matrix`) and a `gRNA_groups_table` and collapses gRNAs in the same group into a single "combined" gRNA. The data frame `gRNA_groups_table` must have columns `gRNA_id` and `gRNA_group`, as above; the `gRNA_type` column is optional.

```{r}
combined_perturbation_matrix <- combine_gRNAs(perturbation_matrix = perturbation_matrix,
...
*Trans* relationships are more complex than *cis* relationships, as *trans* relationships are mediated by one or more transcription factors. Given the resultant uncertainty about the direction of *trans* relationships, a two-sided test is appropriate for all *trans* tests.
...
-   Make a QQ-plot of the negative control pairs (should lie along the diagonal line)
-   Examine the p-values of the positive control pairs.
-   Finally, apply BH to the candidate pairs; report as discoveries those with adjusted p-values \< 0.1.

Note: it is important to apply BH *only* to the candidate pairs (i.e., the pairs in the discovery set); adding positive control and negative control pairs makes BH anticonservative and convservative, respectively.
