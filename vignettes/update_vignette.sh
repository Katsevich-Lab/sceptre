...
sed -i -e 's/parallel = TRUE, n_processors = 2/parallel = sceptre_parallel, n_processors = 2/g' sceptre.Rmd
sed -i -e 's/parallel = TRUE/parallel = sceptre_parallel, n_processors = 2/g' sceptre.Rmd
sed -i -e '/library(Matrix)/a\
sceptre_parallel <- .Platform$OS.type != "windows"
' sceptre.Rmd
