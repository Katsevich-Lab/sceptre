...
vignette: >
  %\VignetteIndexEntry{`sceptre` tutorial}
...
hist_gene <- ggplot(data = example_gene |> tibble(count = _) |> dplyr::filter(count >= 1, count <= 40), mapping = aes(x = count)) +
...
hist_gRNA <- ggplot(data = example_gRNA |> tibble(count = _) |> dplyr::filter(count >= 1, count <= 40), mapping = aes(x = count)) +
