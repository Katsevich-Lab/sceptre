...
`sceptre` is powered under the hood by `ondisc`, an R package that enables fast, universal, and intuitive computing on large-scale single-cell data. If you would like to apply at-scale `sceptre` to your own data, you must (i) convert your gene expression and gRNA perturbation data into `ondisc_matrix` objects, (ii) produce a list of gene-gRNA pairs to analyze, and (iii) compute the cell-specific matrix of technical factors. Please see the tutorial "[Setting up a single-cell pooled CRISPR screen](https://timothy-barry.github.io/ondisc/articles/setting_up_a_crispr_screen.html)," located on the `ondisc` [website](https://timothy-barry.github.io/ondisc/), for instructions on how to do this.
