...
The thresholding method allows for one optional argument, namely the integer threshold `threshold`. An important special case involves setting `threshold` to `1`, which assigns any gRNA expressed in a given cell to that cell. We assign gRNAs to cells on the CRISPRi data using a threshold of 1 below.
...
Note that setting `threshold` to `1` ignores background contamination.
