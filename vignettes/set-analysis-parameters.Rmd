...
This vignette explicatess the "set analysis parameters" step of the `sceptre` pipeline, which involves selecting the parameters that govern how the statistical analysis is to be carried out.
...
We set the analysis parameters by calling the function `set_analysis_parameters()` on the `sceptre_object`. `set_analysis_parameters()` takes several arguments: `positive_control_pairs`, `discovery_pairs`, `side`, `grna_grouping_strategy`, `formula_object`, `control_group`, `resampling_mechanism`, `fit_parametric_curve`, `multiple_testing_method`, and `multiple_testing_alpha`. The only required argument among these is `discovery_pairs`. We describe these arguments below.
...
`construct_cis_pairs()` returns the set of target-response pairs in close physical proximity.
