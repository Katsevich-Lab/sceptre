...
We use `construct_trans_pairs()` to construct the discovery set for the low-MOI CRISPRko dataset, excluding the positive control pairs. The discovery set consists of the set of gene targets paired to *all other* genes.
...
Multiple gRNAs typically are designed to target a given genomic element. The parameter `grna_grouping_strategy` controls if and how gRNAs that target the same genomic element are combined. The default gRNA grouping strategy is `"union"`. `"union"` constructs a "grouped gRNA" by combining all gRNAs that target a given genomic element via a union operation; this "grouped gRNA" is then tested for association against the responses to which the given element is paired. For example, suppose that `grna_1` and `grna_2` target a given genomic element. Suppose that `grna_1` is present in the cells indexed 2, 5, 10, 11, and 19, while `grna_2` is present in the cells indexed 1, 5, 10, 13, and 19.
