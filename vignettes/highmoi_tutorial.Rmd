...
**Update July 2023**: We have released a new, experimental high MOI function. We expect the experimental high MOI function to be faster, more memory efficient, and more statistically powerful than the current high MOI function on most datasets. One can access the documentation for the experimental high MOI function via `?run_sceptre_highmoi_experimental`. We expect to deprecate the current high MOI function (and replace it with the experimental high MOI function) by September 2023.
...
```{r}
pos_control_p_vals <- result |> dplyr::filter(pair_type == "positive_control") |> pull(p_value)
pos_control_p_vals
```
...
```{r}
candidate_pair_results <- result |> dplyr::filter(pair_type == "candidate")
...
```{r}
discovery_set <- candidate_pair_results_p_adj |> dplyr::filter(p_val_adj <= 0.1)
```
