...
1.  **Discovery pairs and positive control pairs**. The primary goal of `sceptre` is to determine whether perturbation of a gRNA target (such as an enhancer) leads to a change in expression of a response (such as gene). We use the term *target-response pair* to refer to a given gRNA target and response that we seek to test for association (upon perturbation of the gRNA target). A *discovery target-response pair* is a target-response pair whose association status we do not know but would like to learn. For example, in an experiment in which we aim to link putative enhancers to genes, the discovery target-response pairs might consist of the set of putative enhancers and genes in close physical proximity to one another.
    ...
    `construct_trans_pairs()` constructs the entire set of target-response pairs and is appropriate when conducting an unbiased screen of all targets and responses. See the help pages of the pair constructor helpers (`?construct_positive_control_pairs()`, `?construct_cis_pairs()`, `?construct_trans_pairs()`) for more information about these functions.
...
We call `print()` on the resulting `sceptre_object`, which prints a detailed summary of the selected analysis parameters (output not displayed here).
...
The third step is to assign gRNAs to cells. This step can be skipped, in which case gRNAs are assigned to cells automatically using default options. The gRNA assignment step involves using the gRNA count matrix to infer which gRNAs are present in which cells. Thus, we begin by plotting the UMI count distribution of several randomly selected gRNAs via a call to the function `plot_grna_count_distributions()`.
...
We can call `plot()` on the resulting `sceptre_object`, which renders a plot summarizing the output of the gRNA-to-cell assignment step.
...
The top panel plots the gRNA-to-cell assignments of three randomly selected gRNAs. Consider one of the plots (e.g., the plot corresponding to `grna_CACACCC`). Each point represents a cell; the y-axis indicates the UMI count of the gRNA in the cell, and the x-axis indicates whether the cell has been classified as "perturbed" (i.e., it *contains* the gRNA) or unperturbed (i.e., it does *not contain* the gRNA). Perturbed (resp., unperturbed) cells are shown in the left (resp., right) column. The bottom left panel is a barplot of the number of cells to which each gRNA has been confidently mapped. Finally, the bottom right panel is a histogram of the number of gRNAs contained in each cell. The mean number of gRNAs per cell --- i.e., the MOI --- is displayed in purple.

We can call `print()` on the `sceptre_object`, which prints a summary of the gRNA-to-cell assignment step and provides an update on the status of the analysis.
...
The cellwise QC that `sceptre` implements is standard. Cells with an extremely high or an extremely low `response_n_nonzero` (i.e., number of expressed responses) value or `response_n_umis` (i.e., number of response UMIs) value are removed. Likewise, cells with an excessively large `response_p_mito` (i.e., fraction of UMIs mapping to mitochondrial genes) value are removed. Additionally, in low-MOI, cells that contain multiple gRNAs (as determined during the RNA-to-cell assignment step) are removed. Finally, users optionally can provide a list of additional cells to remove.

`sceptre` also implements QC at the level of the target-response pair. For a given pair we define the "treatment cells" as those that contain a gRNA targeting the given target. Next, we define the "control cells" as the cells against which the treatment cells are compared to carry out the differential expression test. We define the "number of nonzero treatment cells" (`n_nonzero_trt`) as the number of *treatment* cells with nonzero expression of the response; likewise, we define the "number of nonzero control cells" (`n_nonzero_cntrl`) as the number of *control* cells with nonzero expression of the response. `sceptre` filters out pairs for which `n_nonzero_trt` or `n_nonzero_cntrl` falls below some threshold (by default 7).

We call the function `run_qc()` on the `sceptre_object` to carry out cellwise and pairwise QC. `run_qc()` has several optional arguments that control the stringency of the QC thresholds. For example, we set `p_mito_threshold = 0.1` to remove cells with a `response_p_mito` value exceeding 0.1. The optional arguments are set to reasonable defaults.

```{r}
sceptre_object <- run_qc(sceptre_object,
                         p_mito_threshold = 0.1)
```
...
```{r, out.width = "400px", fig.align="center", echo = FALSE, fig.cap=c("Cellwise and pairwise quality control")}
p <- plot(sceptre_object)
ggplot2::ggsave(filename = "qc.png", plot = p, device = "png", scale = 1.1, width = 3.5, height = 4, dpi = 330)
knitr::include_graphics("qc.png")
```

The top panel is a barplot summarizing the output of the cellwise QC step. The various cellwise QC filters (e.g., "N nonzero responses," "N response UMIs," "Percent mito", etc.) are shown on the x-axis, and the percentage of cells removed due application of a QC filter is shown on the y-axis. Note that a cell could be flagged by multiple QC filters; for example, a cell might have an extremely high `response_n_umi` value *and* an extremely high `response_n_nonzero` value. Thus, the height of the "any filter" bar (which indicates the percentage of cells removed due to application of *any* filter) need not be equal to the sum of the heights of the other bars. The bottom panel is a scatterplot summarizing the output of the pairwise QC step. Each point corresponds to a target-response pair; the y-axis (resp., x-axis) indicates the `n_nonzero_trt` (resp., `n_nonzero_cntrl`) value of that pair. Pairs for which `n_nonzero_trt` or `n_nonzero_cntrl` fall below the threshold are removed (red), while the remaining pairs are retained (green).

Finally, we call `print()` on the updated `sceptre_object`, which prints a summary of the QC step and provides an update on the status of the analysis.
...
The fifth step is to run the calibration check. The calibration check is an analysis that enables us to verify that `sceptre` controls the rate of false discoveries on the dataset under analysis. Negative control target-response pairs are automatically constructed by coupling groups of NT gRNAs to randomly selected responses. Importantly, the negative control pairs are built in such a way that they are highly similar to the discovery pairs, the difference being that the negative control pairs are devoid of biological signal. `sceptre` is then applied to analyze the negative control pairs. Given that the negative control pairs are absent of signal, `sceptre` should produce approximately uniformly distributed p-values on the negative control pairs. Moreover, after an appropriate multiple testing correction, `sceptre` should make zero (or very few) discoveries on the negative control pairs. The calibration check helps us to ensure that the discovery set that `sceptre` ultimately produces is uncontaminated by excess false positives.

We run the calibration check by calling the function `run_calibration_check()` on the `sceptre_object`. (Again, Windows users should remove the `parallel = TRUE` argument.)
...
```{r, out.width = "550px", fig.align="center", echo = FALSE, fig.cap=c("Calibration check results")}
set.seed(3)
p <- plot(sceptre_object)
...
-   The upper left panel is a QQ plot of the p-values on an untransformed scale. The p-values should lie along the diagonal line, indicating uniformity of the p-values in the *bulk* of the distribution.

-   The upper right panel is a QQ plot of the p-values on a negative log-10 transformed scale. The p-values again should lie along the diagonal line (with the majority of the p-values falling within the gray confidence band), indicating uniformity of the p-values in the *tail* of the distribution.
...
-   Finally, the bottom right panel is a text box displaying (i) the number of false discoveries that `sceptre` makes on the negative control data and (ii) the mean estimated log-fold change. The number of false discoveries should be a small integer like zero, one, two, or three, with zero being ideal. The mean estimated log-fold change, meanwhile, should be a numeric value close to zero; a number in the range [-0.1, 0.1] is adequate.

`sceptre` may not exhibit good calibration initially, which is OK. An article discussing strategies for improving the calibration of `sceptre` is forthcoming.

Finally, we can call `print()` on the `sceptre_object` to print a summary of the results of the calibration check and provide an update on the status of the analysis (output suppressed).
...
The sixth step is to run the power check. This step is optional. The power check entails applying `sceptre` to analyze the positive control pairs. Given that we know that there *is* an association between the target and the response for the positive control pairs, `sceptre` should produce highly significant (i.e., small) p-values on these pairs. The power check enables us to verify that `sceptre` is powerful (i.e. capable of detecting true associations) on the dataset under analysis. We run the power check by calling the function `run_power_check()` on the `sceptre_object`.
...
```{r, out.width = "450px", fig.align="center", echo = FALSE, fig.cap=c("Power check results")}
set.seed(3)
p <- plot(sceptre_object)
...
Each point in this plot corresponds to a target-response pair; the y-axis indicates the p-value of a given pair, where smaller (i.e., more significant) p-values are higher. The left (resp., right) column displays the positive control (resp., negative control) p-values. Ideally, the positive control p-values are small, and in particular, smaller than the negative control p-values.

Finally, we can call `print()` on the `sceptre_object` to print information about the power check and track the status of the analysis (output suppressed).
...
The seventh and penultimate step is to run the discovery analysis, which involves applying `sceptre` to analyze the discovery pairs. We run the discovery analysis by calling the function `run_discovery_analysis()` on the `sceptre_object`.
...
```{r, out.width = "550px", fig.align="center", echo = FALSE, fig.cap=c("Discovery analysis results")}
set.seed(3)
p <- plot(sceptre_object)
...
The eighth and final step is to write the outputs of the analysis to a directory on disk. To this end we call the function `write_outputs_to_directory()`, which takes a `sceptre_object` and writes the results contained within the `sceptre_object` to a directory specified by `directory`. We set `directory` to "\~/sceptre_outputs" in this example.

```{r,results='hide'}
write_outputs_to_directory(sceptre_object = sceptre_object, directory = "~/sceptre_outputs")
```
...
We also can obtain the calibration check, power check, and discovery analysis results within R via a call to the function `get_result()`, passing as arguments `sceptre_object` and `analysis`, a string indicating the function whose results we are querying.

```{r}
result <- get_result(sceptre_object, analysis = "run_discovery_analysis")
```

`result` is a data frame, each row of which corresponds to a target-response pair. The columns of `result` are as follows: `response_id`, `grna_target`, `n_nonzero_trt`, `n_nonzero_cntrl`, `pass_qc` (a `TRUE`/`FALSE` value indicating whether the pair passes pairwise QC), `p_value`, `log_2_fold_change`, and `significant` (a `TRUE`/`FALSE` value indicating whether the pair is called as significant).
...
We have written vignettes to describe each step of the pipeline in greater detail.
