...
The third step is to assign gRNAs to cells. This step can be skipped, in which case gRNAs are assigned to cells automatically using default options. The gRNA assignment step involves operating on the gRNA count matrix; thus, we begin by plotting the UMI count distribution of several randomly selected gRNAs via a call to the function `plot_grna_count_distributions()`.
...
We assign gRNAs to cells by calling the function `assign_grnas()`. `assign_grnas()` takes arguments `sceptre_object` (required) and `method` (optional); the latter argument can be set to `"mixture"`, `"maximum"`, or `"thresholding"`. (In this example we use the default method of `"mixture"` in high-MOI.) We parallelize execution of the function by setting `parallel` to `TRUE`. (Windows users should set `parallel` to `FALSE`, as parallel execution is not yet configured for Windows.)
...
We call `plot()` on the resulting `sceptre_object`, which renders a plot summarizing the gRNA-to-cell assignments.
...
The top panel displays plots representing the gRNA-to-cell assignments of three randomly selected gRNAs. Each point in a given plot is a cell; the y-axis indicates the UMI count of the gRNA in that cell, and the x-axis indicates whether that cell has been classified as "perturbed" (i.e., it *contains* the gRNA) or unperturbed (i.e., it does *not contain* the gRNA). Perturbed (resp., unperturbed) cells are shown in the left (resp., right) column. The bottom left panel is a barplot of the number of cells that each gRNA has infected. Finally, the bottom right panel is a histogram of the number of gRNAs contained in each cell. The mean number of gRNAs per cell --- i.e., the MOI --- is displayed in the purple text.

We call `print()` on the `sceptre_object`, which prints some helpful information about the gRNA-to-cell assignments and the status of the analysis (output repressed in vignette).
...
The fourth step is to run quality control (QC). This step likewise can be skipped, in which case QC is applied automatically using default options. `sceptre` implements two kinds of QC: cellwise QC and pairwise QC. Cellwise QC aims to remove low-quality cells, while pairwise QC aims to remove low-quality target-response pairs.

`sceptre` implements standard single-cell cellwise QC. Cells with an extremely high or an extremely low `response_n_nonzero` (i.e., number of expressed responses) value or `response_n_umis` (i.e., number of response UMIs) value are removed. Likewise, cells with an excessively large `response_p_mito` (i.e., fraction of reads mapping to mitochondrial genes) value are removed. Additionally, on low-MOI data, cells that contain multiple gRNAs (as determined by the RNA-to-cell assignment step) are removed. Finally, users optionally can provide a list of additional cells to remove.

`sceptre` also implements QC at the level of the target-response pair. For a given pair we define the "treatment cells" as those that contain a targeting gRNA. Next, we define the "control cells" as the cells against which the treatment cells are compared to carry out the differential expression test. We define the "number of nonzero treatment cells" (`n_nonzero_trt`) as the number of *treatment* cells with nonzero expression of the response; likewise, we define the "number of nonzero control cells" (`n_nonzero_cntrl`) as the number of *control* cells with nonzero expression of the response. `sceptre` filters out pairs for which `n_nonzero_trt` or `n_nonzero_cntrl` falls below some threshold (by default 7).

We carry out QC by calling the function `run_qc`. The arguments to this function control the stringency of the various QC thresholds and are set to reasonable defaults.
...
The top panel displays the output of the cellwise QC step. The various cellwise QC filters (e.g., "Any filter," "N nonzero responses," "N response UMIs," etc.) are shown on the x-axis, and the percentage of cells removed due application of a given QC filter is shown on the y-axis. Note that a cell could be flagged by multiple QC filters; for example, a cell might have an extremely high `response_n_umi` value *and* an extremely high `response_n_nonzero` value. Thus, the height of the "any filter" bar need not be equal to the sum of the heights of the other bars. The bottom panel displays the output of the pairwise QC step. Each point corresponds to a target-response pair; the y-axis (resp., x-axis) indicates the `n_nonzero_trt` (resp., `n_nonzero_cntrl`) value for that pair. Pairs for which `n_nonzero_trt` or `n_nonzero_cntrl` fall below the threshold are removed.

Finally, we call `print()` on the updated `sceptre_object`, which prints to the console information about the QC step.
