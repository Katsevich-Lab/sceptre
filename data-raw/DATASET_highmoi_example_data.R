...
for (i in seq_len(nrow(selected_perturbed_genes))) {
    ...
    starts_i <- gene_tss_i +
        seq.int(0, by = GENE_WINDOW_SHIFT, length.out = GRNAS_PER_GENE)
...
for (i in seq_len(nrow(selected_enhancer_genes))) {
    ...
    candidate_enh_labels <- paste0(
        "candidate_enh_",
        enh_index + seq_len(num_enh_i) - 1
    )

    # each enhancer has GRNAS_PER_ENH gRNAs
    all_enh_df <- list()
    for (j in seq_len(num_enh_i)) {
        starts_j <- enh_starts[j] +
            seq.int(0, by = ENHANCER_GRNA_SHIFT, length.out = GRNAS_PER_ENH)
        ends_j <- starts_j + (ENHANCER_GRNA_SHIFT - 1)

        df_j <- data.frame(
            grna_id = paste0(
                candidate_enh_labels[j],
                "_grna",
                seq_len(GRNAS_PER_ENH)
            ),
...
for (cell_idx in seq_len(NUM_CELLS)) {
    ...
    if (k == 0) {
        ...
        assigned_gRNAs_list[[cell_idx]] <- sample.int(
            NUM_GRNAS,
            size = k,
            replace = FALSE
        )
...
mu_gene[perturbed_gene_ids] <- exp(
    MEAN_LOG_GENE_EXPRESSION + SD_LOG_GENE_EXPRESSION
)
...
for (cell_idx in seq_len(NUM_CELLS)) {
    ...
    if (length(gRNA_rows_in_cell) > 0) {
        grna_effect_matrix[, cell_idx] <-
            colSums(grna_log_fold_change_matrix[
                gRNA_rows_in_cell,
                ,
                drop = FALSE
            ])
...
ondisc:::write_sceptre_object_to_cellranger_format_v2(
    mats = list(
        gene = Matrix(gene_expression_matrix, sparse = TRUE),
