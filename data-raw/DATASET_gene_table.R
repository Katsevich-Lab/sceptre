...
dt <- fread(
    file = "~/research_offsite/external/ref/genes.gtf",
    skip = 5,
    col.names = c(
        "chr",
        "source",
...
gene_table <- cbind(
    dt_gene_chr[, c("chr", "start", "end", "strand")],
    gene_ids_and_names
) |>
...
library(rtracklayer)
dt <- readGFF(
    "~/research_offsite/external/ref/Homo_sapiens.GRCh37.82.gtf.gz"
) |>
    as.data.table()
# retain only genes
...
gene_position_data_frame_grch37$chr <- factor(
