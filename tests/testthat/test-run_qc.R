...
make_mock_base_data_for_testing_run_qc <- function(num_cells) {
    ...
    response_matrix <- make_mock_response_matrices(
        ...
        `rownames<-`(c(
            on_targets,
            paste0("response_", (length(on_targets) + 1):num_responses)
        ))
    ...
    discovery_pairs <- data.frame(
        ...
        response_id = rep(
            paste0("response_", length(on_targets) + 1:2),
            each = 2
        )
    )
...
test_that("run_qc remove cells with multiple grnas in low moi", {
    ...
    num_targets <- sum(
        test_data_list$grna_target_data_frame$grna_target != "non-targeting"
    )
    unique_targets <- unique(test_data_list$grna_target_data_frame$grna_target[
        1:num_targets
    ])
    nt_guides <- with(
        test_data_list$grna_target_data_frame,
    ...
    response_matrix[] <- rep(
        c(0, 0, 1, 0),
        times = prod(dim(response_matrix)) / 4
    )
    for (i in 1:nrow(response_matrix)) {
        response_matrix[i, i] <- 2
    } # to avoid low rank covariate data frame
    ...
    expect_equal(
        ...
        (1:num_grnas)[-locs_of_extra_expression]
    )
})
...
test_that("run_qc remove cells via `additional_cells_to_remove` high moi", {
    ...
    scep_qc <- run_qc(
        scep_high_pre_qc,
        additional_cells_to_remove = cells_to_remove
    )
...
test_that("run_qc remove cells with `response_n_umis_range` and `response_n_nonzero_range` high moi", {
    ...
    scep_qc <- scep_high_pre_qc |>
        run_qc(
            response_n_umis_range = c(0.02, 1),
            response_n_nonzero_range = c(0, 1)
        )
    ...
    scep_qc <- scep_high_pre_qc |>
        run_qc(
            response_n_umis_range = c(0, 1),
            response_n_nonzero_range = c(0.03, 1)
        )
    expect_identical(scep_qc@cells_in_use, (1:100)[-zero_umi_count_idx])
    ...
    scep_qc <- scep_high_pre_qc |>
        run_qc(
            response_n_umis_range = c(0, 1),
            response_n_nonzero_range = c(0, .02)
        )
...
test_that("run_qc remove cells with `p_mito_threshold` high moi", {
    ...
    response_matrix <- test_data_list$response_matrix
    rownames(response_matrix)[6:7] <- paste0(
        "MT-",
        rownames(response_matrix)[6:7]
    )
...
test_that("run_qc test positive control pairs", {
    ...
    grna_matrix <- matrix(
        sample(0:1, num_grna * num_cells, replace = TRUE),
        num_grna,
        num_cells
    ) |>
    ...
    response_matrix <- matrix(
        rpois(num_responses * num_cells, 1),
        num_responses,
        num_cells
    ) |>
    ...
    scep_low_control_nt_all_pass <- scep_low_control_nt_pre_qc |>
        run_qc(
            response_n_umis_range = c(0, 1),
            response_n_nonzero_range = c(0, 1), # don't want to remove any cells here
    ...
    expect_equal(
        scep_low_control_nt_all_pass@positive_control_pairs_with_info$n_nonzero_trt,
        c(10, 10, 0)
    )
    expect_equal(
        scep_low_control_nt_all_pass@positive_control_pairs_with_info$n_nonzero_cntrl,
        c(10, 0, 10)
    )

    # confirming all pass QC with these thresholds
    expect_true(all(
        scep_low_control_nt_all_pass@positive_control_pairs_with_info$pass_qc
    ))
    ...
    expect_equal(
        scep_low_control_nt_fail@positive_control_pairs_with_info$pass_qc,
        c(TRUE, TRUE, FALSE)
    )
    ...
    # confirming the values of `n_nonzero_trt` and `n_nonzero_cntrl`
    expect_equal(
        scep_high_control_complement_all_pass@positive_control_pairs_with_info$n_nonzero_trt,
        c(10, 10, 0)
    )
    expect_equal(
        scep_high_control_complement_all_pass@positive_control_pairs_with_info$n_nonzero_cntrl,
        c(20, 0, 40)
    )

    # confirming all pass QC with these thresholds
    expect_true(all(
        scep_high_control_complement_all_pass@positive_control_pairs_with_info$pass_qc
    ))
    ...
    expect_equal(
        scep_high_control_complement_fail@positive_control_pairs_with_info$pass_qc,
        c(TRUE, TRUE, FALSE)
    )
...
test_that("run_qc test discovery pairs", {
    ...
    # confirming the values of `n_nonzero_trt` and `n_nonzero_cntrl`
    expect_equal(
        scep_low_control_nt_all_pass@discovery_pairs_with_info$n_nonzero_trt,
        c(10, 0, 0)
    )
    expect_equal(
        scep_low_control_nt_all_pass@discovery_pairs_with_info$n_nonzero_cntrl,
        c(0, 10, 10)
    )

    # confirming all pass QC with these thresholds
    expect_true(all(
        scep_low_control_nt_all_pass@discovery_pairs_with_info$pass_qc
    ))
    ...
    expect_equal(
        scep_low_control_nt_fail@discovery_pairs_with_info$pass_qc,
        c(TRUE, FALSE, FALSE)
    )
    ...
    # confirming the values of `n_nonzero_trt` and `n_nonzero_cntrl`
    expect_equal(
        scep_high_control_complement_all_pass@discovery_pairs_with_info$n_nonzero_trt,
        c(10, 0, 0)
    )
    expect_equal(
        scep_high_control_complement_all_pass@discovery_pairs_with_info$n_nonzero_cntrl,
        c(10, 30, 40)
    )

    # confirming all pass QC with these thresholds
    expect_true(all(
        scep_high_control_complement_all_pass@discovery_pairs_with_info$pass_qc
    ))
    ...
    expect_equal(
        scep_high_control_complement_fail@discovery_pairs_with_info$pass_qc,
        c(TRUE, FALSE, FALSE)
    )
...
test_that("run_qc PC pairs interaction between cellwise QC with `response_n_umis_range` and `response_n_nonzero_range`, and pairwise QC low moi", {
    ...
    scep_low <- import_data(
        ...
        run_qc(
            response_n_umis_range = c(0, .85),
            response_n_nonzero_range = c(0.20, 1),
    ...
    expect_setequal(
        ...
        all_cells[
            -c(
                cells_to_remove_low_umi,
                cells_to_remove_high_umi,
                cells_expressing_no_grna
            )
        ]
    )
...
test_that("run_qc discovery pairs interaction between cellwise QC with `response_n_umis_range` and `response_n_nonzero_range`, and pairwise QC high moi", {
    ...
    scep_high <- import_data(
        ...
        run_qc(
            response_n_umis_range = c(0, .85),
            response_n_nonzero_range = c(.20, 1),
            n_nonzero_trt_thresh = 0,
            n_nonzero_cntrl_thresh = 11
        )
