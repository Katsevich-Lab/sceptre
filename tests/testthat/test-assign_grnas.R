...
make_mock_base_data_for_testing_assign_grnas <- function() {
    ...
    discovery_pairs <- data.frame(
        ...
        response_id = rep(
            paste0("response_", length(on_targets) + 1),
            times = 2
        )
    )
...
test_that("assign_grnas method=maximum moi=low grna_matrix all 1", {
    ...
    expect_equal(
        ...
        lapply(
            test_data_list$grna_target_data_frame$grna_id,
            function(target_name) {
                if (
                    target_name ==
                        test_data_list$grna_target_data_frame$grna_id[1]
                ) {
                    1:num_cells
                } else {
                    integer(0)
    ...
    guide_part_all_1 <- lapply(unique_targets, function(target_name) {
        if (
            target_name == test_data_list$grna_target_data_frame$grna_target[1]
        ) {
            1:num_cells
        } else {
            integer(0)
...
test_that("assign_grnas method=maximum moi=low grna_matrix clear max", {
    ...
    expect_equal(
        scep_low_clear_max@cells_w_zero_or_twoplus_grnas,
        (num_grnas + 1):num_cells
    )
...
test_that("assign_grnas method=maximum moi=low grna_matrix varying umi_fraction_threshold", {
    ...
    expect_equal(
        scep_low_with_low_frac@cells_w_zero_or_twoplus_grnas,
        integer(0)
    )
...
test_that("assign_grnas method=threshold moi=low", {
    ...
    expect_equal(
        scep_low_with_low_thresh@cells_w_zero_or_twoplus_grnas,
        1:num_cells
    )

    expect_equal(
        scep_low_with_low_thresh@initial_grna_assignment_list,
        lapply(1:num_grnas, function(i) {
            if (i %in% expressed_grna_ids) 1:num_cells else integer(0)
        }) |>
            setNames(test_data_list$grna_target_data_frame$grna_id)
    )

    # the check is `i == 3` because the target with expressed guides is t1_c3_d1, the 3rd one
    guide_part <- lapply(seq_along(unique_targets), function(i) {
        if (i == 3) 1:num_cells else integer(0)
    }) |>
        setNames(unique_targets)
    ...
    expect_equal(
        scep_low_with_high_thresh@cells_w_zero_or_twoplus_grnas,
        integer(0)
    )
    ...
    expect_equal(
        ...
        lapply(1:num_grnas, function(i) {
            if (i == 5) 1:num_cells else integer(0)
        }) |>
            setNames(test_data_list$grna_target_data_frame$grna_id)
    )
...
test_that("assign_grnas method=threshold moi=high", {
    ...
    expect_equal(
        ...
        lapply(1:num_grnas, function(i) {
            if (i <= 2) 1:num_cells else integer(0)
        }) |>
            setNames(test_data_list$grna_target_data_frame$grna_id)
    )
    ...
    expect_equal(
        ...
        lapply(1:num_grnas, function(i) {
            if (i == 1) 1:num_cells else integer(0)
        }) |>
            setNames(test_data_list$grna_target_data_frame$grna_id)
    )
})
...
test_that("assign_grnas method=mixture moi=high", {
    ...
    expect_equal(
        scep_high_mixture@initial_grna_assignment_list$grna_1,
        cells_getting_grna_1
    )
    expect_equal(
        scep_high_mixture@initial_grna_assignment_list$grna_2,
        cells_getting_grna_2
    )
    expect_equal(
        scep_high_mixture@initial_grna_assignment_list$grna_3,
        cells_getting_grna_3
    )
    expect_equal(
        scep_high_mixture@initial_grna_assignment_list$nt_1,
        cells_getting_nt_1
    )
})
