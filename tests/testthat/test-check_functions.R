...
test_that("check_import_data_inputs", {
    ...
    valid_response_matrix <- make_mock_response_matrices(
        num_responses,
        num_cells,
        patterns = "one"
    )
    valid_grna_matrix <- make_mock_grna_matrices(
        valid_grna_target_data_frame,
        num_cells,
        non_nt_patterns = "one",
        nt_patterns = "one"
    )
    # this is a nice one with basically balanced levels
    valid_extra_covariates <- make_mock_extra_covariates_data_frames(
        num_cells,
        patterns = "many_levels"
    )
        ...
        check_import_data_inputs(
            ...
            grna_target_data_frame = valid_grna_target_data_frame |>
                ...
                .Primitive("[<-")(
                    3,
                    1,
                    "this grna id is not in the grna matrix"
                ),
            moi = "low",
            extra_covariates = valid_extra_covariates
        ),
    ...
    expect_no_error(
        check_import_data_inputs(
            ...
            extra_covariates = valid_extra_covariates_mismatched_rownames |>
                `rownames<-`(colnames(
                    valid_response_matrix_mismatched_colnames
                ))
        )
    )
    ...
    expect_error(
        check_import_data_inputs(
            ...
            grna_target_data_frame = valid_grna_target_data_frame,
            moi = "low",
            extra_covariates = make_mock_extra_covariates_data_frames(
                num_cells,
    ...
    expect_error(
        check_import_data_inputs(
            ...
            extra_covariates = make_mock_extra_covariates_data_frames(
                ...
                dplyr::mutate(
                    list_col = c(list(c(1, 1, 1)), as.list(2:num_cells))
                )
        ),
