...
test_that("parallel argument must be a single logical value", {
  expect_error(check_parallel_supported(NA), "`parallel` should be a single logical value.", fixed = TRUE)
  expect_error(check_parallel_supported(c(TRUE, FALSE)), "`parallel` should be a single logical value.", fixed = TRUE)
  expect_error(check_parallel_supported("TRUE"), "`parallel` should be a single logical value.", fixed = TRUE)
})
