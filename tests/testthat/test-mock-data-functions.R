...
test_that(".make_mock_cross_target_chr_with_distances", {
    ...
    for (target_and_chr_input in target_and_chr_data_inputs) {
        for (chr_distance_and_start_input in chr_distance_and_start_inputs) {
            target_and_chr <- .make_mock_target_and_chr_only(
                target_and_chr_input
            )

            out <- .make_mock_cross_target_chr_with_distances(
                target_and_chr_data = target_and_chr,
            ...
            expect_equal(
                nrow(out),
                sum(target_and_chr_input) *
                    2 *
                    length(chr_distance_and_start_input$distance)
            )
            expect_equal(
                length(unique(out$chr)),
                (length(target_and_chr_input) + 1) *
                    length(chr_distance_and_start_input$distance)
            )
            expect_equal(
                length(unique(out$grna_target)),
                length(target_and_chr_input) *
                    2 *
                    length(chr_distance_and_start_input$distance)
            )
            ...
            expect_equal(
                out$start,
                chr_distance_and_start_input$start[target_inds]
            )
            expect_equal(
                out$end - out$start,
                chr_distance_and_start_input$distance[target_inds]
            )
            ...
            expect_equal(
                ...
                rep(
                    target_and_chr_target_starts,
                    times = length(chr_distance_and_start_input$distance)
                )
            )
        }
    }
    ...
    for (target_and_chr_input in target_and_chr_data_inputs) {
        for (chr_distance_and_start_input in chr_distance_and_start_inputs) {
            for (num_nt in num_nts) {
                ...
                out_pieces <- split(
                    out,
                    ifelse(out$grna_target == "non-targeting", "nt", "non_nt")
                )

                if (num_nt > 0) {
                    expect_equal(nrow(out_pieces$nt), num_nt)
                    expect_equal(
                        sub("nt", "", out_pieces$nt$grna_id) |> as.numeric(),
                        1:num_nt
                    )
                }
                expect_equal(
...
test_that(".make_mock_patterned_matrix", {
    ...
    for (num_rows in c(1, 5, 12, 13, 14, 25, 26, 27)) {
        for (num_cols in c(1, 5, 12, 13, 14, 25, 26, 27)) {
            out_cols <- .make_mock_patterned_matrix(
                num_rows,
                num_cols,
                patterns_at_col_level = TRUE,
                big = big_test
            )
            out_rows <- .make_mock_patterned_matrix(
                num_rows,
                num_cols,
                patterns_at_col_level = FALSE,
                big = big_test
            )
            ...
            if (num_cols >= 6) {
                expect_equal(
                    out_cols[, 6],
                    rep(c(big_test, 0), c(num_rows - 1, 1))
                )
            }
            if (num_cols >= 7) {
                expect_equal(
                    out_cols[, 7],
                    rep(c(big_test, 1), c(num_rows - 1, 1))
                )
            }
            if (num_cols >= 8) {
                expect_equal(
                    out_cols[, 8],
                    rep(c(0, big_test), c(num_rows - 1, 1))
                )
            }
            if (num_cols >= 9) {
                expect_equal(
                    out_cols[, 9],
                    rep(c(1, big_test), c(num_rows - 1, 1))
                )
            }
            if (num_cols >= 10) {
            ...
            if (num_cols >= 12) {
                expect_equal(
                    out_cols[, 12],
                    seq(0, big_test, length = num_rows) |> round()
                )
            }
            if (num_cols >= 13) {
                expect_equal(
                    out_cols[, 13],
                    seq(big_test, 1, length = num_rows) |> round()
                )
            }
            ...
            if (num_rows >= 6) {
                expect_equal(
                    out_rows[6, ],
                    rep(c(big_test, 0), c(num_cols - 1, 1))
                )
            }
            if (num_rows >= 7) {
                expect_equal(
                    out_rows[7, ],
                    rep(c(big_test, 1), c(num_cols - 1, 1))
                )
            }
            if (num_rows >= 8) {
                expect_equal(
                    out_rows[8, ],
                    rep(c(0, big_test), c(num_cols - 1, 1))
                )
            }
            if (num_rows >= 9) {
                expect_equal(
                    out_rows[9, ],
                    rep(c(1, big_test), c(num_cols - 1, 1))
                )
            }
            if (num_rows >= 10) {
            ...
            if (num_rows >= 12) {
                expect_equal(
                    out_rows[12, ],
                    seq(0, big_test, length = num_cols) |> round()
                )
            }
            if (num_rows >= 13) {
                expect_equal(
                    out_rows[13, ],
                    seq(big_test, 1, length = num_cols) |> round()
                )
            }
...
test_that(".make_mock_matrix_pattern_list", {
    ...
    for (dims in dims_to_test) {
        ...
        results_zero <- .make_mock_matrix_pattern_list(
            patterns_to_make = "zero",
            pattern_nrow = curr_nrow,
            pattern_ncol = curr_ncol,
            big = big,
            pattern_rownames = curr_rownames
        )
...
test_that("make_mock_grna_matrices", {
    ...
    expect_equal(
        dim(results_no_nt_one),
        c(nrow(grna_target_data_frame_no_nt), num_cells)
    )
    expect_true(identical(
        rownames(results_no_nt_one),
        grna_target_data_frame_no_nt$grna_id
    ))
    ...
    expect_equal(
        dim(results_with_nt_single),
        c(nrow(grna_target_data_frame_with_nt), num_cells)
    )
    expect_true(identical(
        rownames(results_with_nt_single),
        grna_target_data_frame_with_nt$grna_id
    ))
    set.seed(321)
    expect_equal(
        results_with_nt_single[
            grna_target_data_frame_with_nt$grna_target != "non-targeting",
        ] |>
            as.matrix() |>
            `attr<-`("dimnames", NULL),
        .make_mock_patterned_matrix(
            sum(grna_target_data_frame_with_nt$grna_target != "non-targeting"),
            num_cells,
            patterns_at_col_level = TRUE,
            big = big
        )
    ...
    expect_equal(
        names(results_no_nt_several),
        paste0("non_nt_", c("one", "row", "column"))
    )

    expect_true(all(sapply(results_no_nt_several, function(mat) {
        dim(mat) == c(nrow(grna_target_data_frame_no_nt), num_cells)
    })))
    expect_true(all(sapply(results_no_nt_several, function(mat) {
        identical(rownames(mat), grna_target_data_frame_no_nt$grna_id)
    })))
    set.seed(123)
    expect_equal(
        results_no_nt_several$non_nt_row |>
            as.matrix() |>
            `attr<-`("dimnames", NULL),
        .make_mock_patterned_matrix(
            nrow(grna_target_data_frame_no_nt),
            num_cells,
            patterns_at_col_level = FALSE,
            big = big
        )
    ...
    all_names <- outer(
        paste0("non_nt_", pattern_names, "_"),
        paste0("nt_", pattern_names),
        paste0
    ) |>
        t() |>
        as.character()
    ...
    expect_true(all(sapply(results_with_nt_all, function(mat) {
        dim(mat) == c(nrow(grna_target_data_frame_with_nt), num_cells)
    })))
    expect_true(all(sapply(results_with_nt_all, function(mat) {
        identical(rownames(mat), grna_target_data_frame_with_nt$grna_id)
    })))
...
test_that("make_mock_extra_covariates_data_frames", {
    ...
    all_names <- c(
        "constant",
        "almost_constant_two_levels_one_val",
        "almost_constant_two_levels_two_vals",
        "almost_constant_many_levels_one_val",
        "almost_constant_many_levels_two_vals",
        "many_levels",
        "missing_level",
        "numeric",
        "count",
        "factor_one_value_level",
        "factor_many_levels",
        "many_columns"
    )
    ...
    for (num_cells in num_cell_values) {
        set.seed(num_cells)
        results_all <- make_mock_extra_covariates_data_frames(
            num_cells,
            patterns = "all"
        )
        ...
        expect_equal(
            sapply(results_all, ncol) |> as.numeric(),
            c(1, 1, 1, 1, 1, 1, 1, 2, 2, 2, 2, 4)
        )
        expect_equal(
            sapply(results_all, nrow) |> as.numeric(),
            rep(num_cells, 12)
        )

        expect_equal(
            results_all$constant$batch,
            rep("b1", num_cells) |> factor(levels = "b1")
        )
        expect_equal(
            table(results_all$almost_constant_two_levels_one_val$batch) |>
                as.numeric(),
            c(1, num_cells - 1)
        )
        expect_equal(
            table(results_all$almost_constant_two_levels_two_vals$batch) |>
                as.numeric(),
            c(2, num_cells - 2)
        )
        expect_equal(
            table(results_all$almost_constant_many_levels_one_val$batch) |>
                as.numeric(),
            rep(c(1, num_cells - 9), c(9, 1))
        )
        expect_equal(
            table(results_all$almost_constant_many_levels_two_vals$batch) |>
                as.numeric(),
            rep(c(2, num_cells - 18), c(9, 1))
        )
        expect_equal(
            table(results_all$many_levels$batch) |> as.numeric(),
            rep(c(num_cells %/% 5, num_cells %/% 5 + num_cells %% 5), c(4, 1))
        )
        expect_equal(
            table(results_all$missing_level$batch) |> as.numeric(),
            c(num_cells %/% 2, num_cells %/% 2 + num_cells %% 2, 0)
        )

        expect_equal(
            table(results_all$factor_one_value_level$batch) |> as.numeric(),
            c(
                num_cells %/% 3,
                num_cells %/% 3,
        ...
        expect_equal(
            table(results_all$factor_many_levels$factor) |> as.numeric(),
            num_cells %/% 5 + (num_cells %% 5) * rep(0:1, c(4, 1))
        )
        expect_equal(
            table(results_all$many_columns$factor) |> as.numeric(),
            num_cells %/% 5 + (num_cells %% 5) * rep(0:1, c(4, 1))
        )
    }
    ...
    set.seed(num_cells)
    results_single_df <- make_mock_extra_covariates_data_frames(
        num_cells,
    ...
    set.seed(num_cells)
    results_several_df <- make_mock_extra_covariates_data_frames(
        num_cells,
        patterns = c(
            "many_columns",
            "constant",
