...
test_that("import_data", {
    ...
    grna_matrix <- matrix(
        rpois(nrow(grna_target_data_frame) * num_cells, 1),
        ncol = num_cells
    ) |>
    ...
    all_slots <- c(
        ...
        "cell_removal_metrics",
        "cellwise_qc_thresholds",
        "mitochondrial_gene",
        "M_matrix",
        "n_nonzero_tot_vector",
        "discovery_pairs_with_info",
        "positive_control_pairs_with_info",
        "negative_control_pairs",
        "initial_grna_assignment_list",
        "grna_assignments_raw",
        "grna_assignments",
        "grnas_per_cell",
        "cells_w_zero_or_twoplus_grnas",
        "cells_in_use",
        "n_discovery_pairs",
        "n_ok_discovery_pairs",
        "n_positive_control_pairs",
        "n_ok_positive_control_pairs",
        "calibration_group_size",
        "n_calibration_pairs",
        "import_grna_assignment_info",
        "mean_cells_per_grna",
    ...
    expect_equal(
        slotNames(sceptre_object_low_no_ec_with_response_names),
        all_slots
    )
    ...
    expect_equal(
        get_response_matrix(sceptre_object_high_with_ec),
        set_matrix_accessibility(response_matrix)
    )
    expect_equal(
        get_grna_matrix(sceptre_object_high_with_ec),
        set_matrix_accessibility(grna_matrix)
    )
    ...
    expect_equal(
        sceptre_object_low_no_ec_with_response_names@covariate_data_frame |>
            rownames(),
    ...
    expect_equal(
        sceptre_object_high_with_ec@last_function_called,
        "import_data"
    )
    expect_equal(
        names(sceptre_object_high_with_ec@functs_called),
        c(
            "import_data",
    ...
    expect_equal(
        sceptre_object_high_with_ec@functs_called |> as.logical(),
        rep(c(TRUE, FALSE), c(1, 6))
    )
})
...
test_that("set_analysis_parameters", {
    ...
    expect_true(
        sceptre_object_post_all_defaults_high_moi@control_group_complement
    )
    expect_false(
        sceptre_object_post_all_defaults_low_moi@control_group_complement
    )
    ...
    expect_equal(
        as.character(sceptre_object_post_all_defaults_high_moi@formula_object)[
            2
        ],
