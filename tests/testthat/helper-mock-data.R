...
.make_mock_target_and_chr_only <- function(num_guides_per_target) {
    targets <- rep(
        paste0("t", 1:length(num_guides_per_target)),
        num_guides_per_target
    )
    chrs <- rep(
        paste0("c", 1:(length(num_guides_per_target) + 1)),
        c(num_guides_per_target, sum(num_guides_per_target))
    )
    grna_target_data_simple <- data.frame(
        grna_target = c(
            paste0(
                "t1_c",
                rep(1:length(num_guides_per_target), num_guides_per_target)
            ),
...
.make_mock_cross_target_chr_with_distances <- function(
    target_and_chr_data,
    chr_distances,
    chr_starts
) {
    target_chr_and_dist_data <- lapply(seq_along(chr_distances), function(i) {
        target_and_chr_data |>
            dplyr::transmute(
                ...
                chr = paste0(chr, "_d", i),
                start = chr_starts[i],
                end = chr_starts[i] + chr_distances[i]
            )
    }) |>
        do.call(what = rbind)
...
make_mock_grna_target_data <- function(
    num_guides_per_target,
    chr_distances,
    chr_starts,
    num_nt_guides
    ...
    if (length(chr_starts) != length(chr_distances)) {
        stop(
            "`chr_starts` must be length 1 or have the same length as `chr_distances`."
        )
    }
    ...
    grna_target_data <- .make_mock_cross_target_chr_with_distances(
        ...
        chr_distances = chr_distances,
        chr_starts = chr_starts
    )
    ...
    target_labels <- with(
        grna_target_data,
        grna_target[!duplicated(grna_target)]
    )
    # This vector enumerates each unique target: we get a sequence 1:(# times that target appears)
    grna_id_number <- table(grna_target_data$grna_target)[target_labels] |>
        sapply(function(i) 1:i) |>
        unlist()
...
# Pois(1) noise.
.make_mock_patterned_matrix <- function(
    num_rows,
    num_cols,
    patterns_at_col_level = TRUE,
    big = 1000
) {
    dims <- if (patterns_at_col_level) {
        c(num_rows, num_cols)
    } else {
        c(num_cols, num_rows)
    }
    ...
    patterned_matrix <- cbind(
        ...
        1,
        big, # constant
        rep(c(0, 1), c(dims[1] - 1, 1)),
        rep(c(0, 1), c(1, dims[1] - 1)), # all but one element constant
        rep(c(big, 0), c(dims[1] - 1, 1)),
        rep(c(big, 1), c(dims[1] - 1, 1)),
        rep(c(0, big), c(dims[1] - 1, 1)),
        rep(c(1, big), c(dims[1] - 1, 1)),
        0:(dims[1] - 1),
        dims[1]:1, # sequence columns
        seq(0, big, length = dims[1]) |> round(),
        seq(big, 1, length = dims[1]) |> round()
    ) |>
    ...
    if (dims[2] > num_patterns) {
        ...
        if (num_extra_cols > 0) {
            patterned_matrix <- cbind(
                patterned_matrix,
                matrix(
                    rpois(num_extra_cols * dims[1], 1),
                    ncol = num_extra_cols
                ) # appending noise
            )
        }
    } else if (dims[2] < 13) {
        # trim down to however many columns were asked for
...
.make_mock_matrix_pattern_list <- function(
    patterns_to_make,
    pattern_nrow,
    pattern_ncol,
    big,
    ...
    patterns_to_make <- intersect(
        c("zero", "one", "big", "row", "column"),
        patterns_to_make
    )
    pattern_list <- list()
    if ("zero" %in% patterns_to_make) {
        # matrix with all 0
        pattern_list <- c(
            pattern_list,
            matrix(0, pattern_nrow, pattern_ncol) |> list()
        )
    }
    if ("one" %in% patterns_to_make) {
        # matrix with all 1
        pattern_list <- c(
            pattern_list,
            matrix(1, pattern_nrow, pattern_ncol) |> list()
        )
    }
    if ("big" %in% patterns_to_make) {
        # matrix with all `big`
        pattern_list <- c(
            pattern_list,
            matrix(big, pattern_nrow, pattern_ncol) |> list()
        )
    }
    if ("row" %in% patterns_to_make) {
        # matrix with patterns in the rows
        pattern_list <- c(
            pattern_list,
    ...
    if ("column" %in% patterns_to_make) {
        # matrix with patterns in the columns
        pattern_list <- c(
            pattern_list,
            .make_mock_patterned_matrix(
                pattern_nrow,
...
#' @examples
#' # See `?make_mock_response_matrix_list` for a full example using all of these functions.
make_mock_grna_matrices <- function(
    grna_target_data_frame,
    num_cells,
    non_nt_patterns,
    nt_patterns = NULL,
    big = 10000,
    return_as_sparse = TRUE,
    unlist_if_single_pattern = TRUE
) {
    allowed_patterns <- c("all", "zero", "one", "big", "row", "column")
    if (
        !all(non_nt_patterns %in% allowed_patterns) ||
            length(non_nt_patterns) == 0
    ) {
        stop(paste0(
            '`non_nt_patterns` must be a non-empty subset of c("',
            paste0(allowed_patterns[-1], collapse = '", "'),
            '") or be equal to "',
            allowed_patterns[1],
            '".'
        ))
    ...
    non_nt_ids <- with(
        grna_target_data_frame,
        grna_id[grna_target != "non-targeting"]
    )

    if (num_nt > 0) {
        if (is.null(nt_patterns)) {
            stop(
                "`grna_target_data_frame` contains non-targeting gRNA but `nt_patterns` has not been set."
            )
        }
        if (
            !all(nt_patterns %in% allowed_patterns) ||
                length(non_nt_patterns) == 0
        ) {
            stop(paste0(
                '`nt_patterns` must be a non-empty subset of c("',
                paste0(allowed_patterns[-1], collapse = '", "'),
                '") or be equal to "',
                allowed_patterns[1],
                '".'
    ...
    if (num_nt == 0) {
        ...
        final_pattern_list <- vector(
            "list",
            length(non_nt_pattern_list) * length(nt_pattern_list)
        )
        k <- 1
        for (i in seq_along(non_nt_pattern_list)) {
            for (j in seq_along(nt_pattern_list)) {
                final_pattern_list[[k]] <- rbind(
                    non_nt_pattern_list[[i]],
                    nt_pattern_list[[j]]
                )
                names(final_pattern_list)[k] <- paste0(
                    names(non_nt_pattern_list)[i],
                    "_",
                    names(nt_pattern_list)[j]
                )
                k <- k + 1
            }
        }
...
make_mock_response_matrices <- function(
    num_responses,
    num_cells,
    patterns,
    big = 10000,
    ...
    if (!all(patterns %in% allowed_patterns) || length(patterns) == 0) {
        stop(paste0(
            '`patterns` must be a non-empty subset of c("',
            paste0(allowed_patterns[-1], collapse = '", "'),
            '") or be equal to "',
            allowed_patterns[1],
            '".'
...
make_mock_extra_covariates_data_frames <- function(
    num_cells,
    patterns,
    unlist_if_single_pattern = TRUE
) {
    ...
    three_level_batch <- data.frame(
        batch = rep(
            c("b1", "b2", "b3"),
            times = num_cells %/% 3 + c(0, 0, num_cells %% 3)
        ) |>
    ...
    if ("constant" %in% patterns) {
        pattern_list <- c(
            pattern_list,
            data.frame(batch = rep("b1", num_cells) |> factor(levels = "b1")) |>
                list()
        )
    }
    if ("almost_constant_two_levels_one_val" %in% patterns) {
        pattern_list <- c(
            pattern_list,
            data.frame(
                batch = rep(c("b1", "b2"), c(1, num_cells - 1)) |>
    ...
    if ("almost_constant_two_levels_two_vals" %in% patterns) {
        pattern_list <- c(
            pattern_list,
            data.frame(
                batch = rep(c("b1", "b2"), c(2, num_cells - 2)) |>
                    factor(levels = c("b1", "b2"))
            ) |>
                list()
        )
    ...
    if ("missing_level" %in% patterns) {
        ...
        pattern_list <- c(
            ...
            data.frame(
                batch = rep(
                    c("b1", "b2"),
                    c(num_cells %/% 2, num_cells %/% 2 + num_cells %% 2)
                ) |>
                    factor(levels = c("b1", "b2", "b3"))
            ) |>
                list()
    ...
    if ("numeric" %in% patterns) {
        pattern_list <- c(
            pattern_list,
            three_level_batch |>
                dplyr::mutate(numeric = rnorm(num_cells)) |>
                list()
        )
    }
    if ("count" %in% patterns) {
        pattern_list <- c(
            pattern_list,
            three_level_batch |>
                dplyr::mutate(count = rpois(num_cells, 2)) |>
                list()
        )
    }
        ...
        pattern_list <- c(
            ...
            three_level_batch |>
                dplyr::mutate(
                    factor = rep(
                        paste0("fac", 1:5),
                        num_cells %/% 5 + c(0, 0, 0, 0, num_cells %% 5)
                    ) |>
    ...
    if ("many_columns" %in% patterns) {
        pattern_list <- c(
            ...
            three_level_batch |>
                dplyr::mutate(
                    numeric = rnorm(num_cells),
                    count = rpois(num_cells, 2),
                    factor = rep(
                        paste0("fac", 1:5),
                        num_cells %/% 5 + c(0, 0, 0, 0, num_cells %% 5)
                    ) |>
